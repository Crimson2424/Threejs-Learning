...
void main()
{
    ...
    //Pattern 13
    // float strenght = step(0.8, mod(vUv.y * 10.0,1.0));
    // strenght *= step(0.5, mod(vUv.x * 10.0,1.0));
    ...
    //Pattern 19
    // float strenght = step(0.2,max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));
    // strenght *= 1.0 - step(0.25, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));
    ...
    // angle /= PI * 2.0;
    // angle += 0.5;
    // float strenght = mod(angle * 20.0 , 1.0);
    ...
    //Pattern 46
    // float strenght = step(0.01 ,cnoise(vUv * 10.0));
